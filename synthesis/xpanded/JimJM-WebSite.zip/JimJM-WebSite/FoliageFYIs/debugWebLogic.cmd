...
@rem
@rem JAVA_SDK       - Pathname of Java SDK (jdb lives in bin directory) 
...
@rem Check the user defined variables, if they are not present, assume nothing
@rem and bail out.
if "%JAVA_SDK%" == "" goto BailAndFail
...
:wrongplace
echo debugWebLogic.cmd: must be run from the WebLogic installation directory. 1>&2
goto finish
...
:runWebLogicJava
set WEBLOGIC_CLASSPATH=.\license;.\classes;.\lib\weblogicaux.jar;.\lib\persistence\Weblogic_RDBMS.jar;.\myserver\serverclasses;%DPI_HOME%\Code\Spider.jar
...
%JAVA_SDK%\bin\java -Djava.compiler=NONE -Xms64m -Xmx64m -classpath %JAVA_CLASSPATH% -Dweblogic.class.path=%WEBLOGIC_CLASSPATH% -Dweblogic.home=. -Djava.security.manager -Djava.security.policy==.\weblogic.policy -Xdebug -Xnoagent -Xrunjdwp:transport=dt_socket,server=y,suspend=y,address=7081 weblogic.Server
goto finish

:BailAndFail
echo *
echo * You must define both JAVA_SDK and DPI_HOME in your
